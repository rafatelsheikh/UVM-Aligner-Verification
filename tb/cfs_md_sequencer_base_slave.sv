`ifndef CFS_MD_SEQUENCER_BASE_SLAVE_SV
    ...
    class cfs_md_sequencer_base_slave extends cfs_md_sequencer_base#(.ITEM_DRV(cfs_md_item_drv_slave));
        ...
        // constructor
        function new(string name = "cfs_md_sequencer_base_slave", uvm_component parent);
        ...
        // write function to write from the monitor port to the pending items fifo
        virtual function void write(cfs_md_item_mon item);
            if (item.is_in_progress) begin
                if (pending_items.is_full()) begin
                    `uvm_fatal("ALGORITHM_ISSUE", $sformatf("FIFO %0s is full (size: %0d)", pending_items.get_full_name(), pending_items.size()))
                end
